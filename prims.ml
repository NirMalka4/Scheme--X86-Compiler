...
"_set_car:
	...
	push rdi
	push rsi
	mov rdi, PVAR(0); points to pair
	mov rsi, PVAR(1); points to value
	mov qword [rdi + TYPE_SIZE], rsi
	pop rsi
	pop rdi
...
"_set_cdr:
	...
	mov rdi, PVAR(0); points to pair
	mov rsi, PVAR(1); points to value
	mov qword [rdi + TYPE_SIZE + WORD_SIZE], rsi
	pop rsi
	pop rdi
	mov rax, SOB_VOID_ADDRESS
...
"_apply:
  ...
  dec rcx
  mov rsi, PVAR(rcx) ; address of lst
  PUSH_APPLY_LIST_ELEMENTS rsi, rdi, rbx
  dec rcx
  PUSH_APPLY_ELEMENTS rcx; push |PARAMS_COUNT| - 2 => skip proc and lst

  add rbx, rcx ; number of arguments pushed into proc frame
  push rbx

  mov r9, PVAR(0) ; closure of proc
  CLOSURE_ENV rdx, r9
  push rdx

  push qword [rbp + 1 * WORD_SIZE] ; old return address to the procedure called apply

  add rbx, 4 ; size of proc frame (K args, K, proc env, apply ret address, magic)
  SHIFT_FRAME rbx

  CLOSURE_CODE rdx, r9
  jmp rdx
";;
  let procs = String.concat "\n\n" [type_queries ; numeric_ops; misc_ops; cons; car; cdr; set_car; set_cdr; apply;];;
end;;
